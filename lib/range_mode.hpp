#ifndef LIB_RANGE_MODE_HPP
#define LIB_RANGE_MODE_HPP 1
...
struct range_mode {
    ...
    range_mode() {}
    explicit range_mode(const std::vector<T> &v) { build(v); }
    ...
    void build(const std::vector<T> &v) {
        ...
        rank.resize(n);
        pos.resize(n);
        large.resize(n / sq + 3, std::vector<std::pair<i32, i32>>(n / sq + 3));
