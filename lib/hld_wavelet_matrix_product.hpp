...
template <typename T, typename RangeProduct> struct hld_wavelet_matrix_product {
    ...
    std::pair<i32, X> count_subtree(i32 u, T a) const { return wm.count(h.tin[u], h.tin[u] + h.sz[u], a); }

    std::pair<i32, X> count_subtree(i32 u, T a, T b) const { return wm.count(h.tin[u], h.tin[u] + h.sz[u], a, b); }
    ...
    std::pair<T, X> kth_subtree(i32 u, i32 k) const { return wm.kth(h.tin[u], h.tin[u] + h.sz[u], k); }

    void set(i32 u, X x) { wm.set(h.tin[u], x); }

    void multiply(i32 u, X x) { wm.mutiply(h.tin[u], x); }
};
