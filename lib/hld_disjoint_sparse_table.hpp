#ifndef LIB_HLD_DISJOINT_SPARSE_TABLE_HPP
#define LIB_HLD_DISJOINT_SPARSE_TABLE_HPP 1
...
template <typename Monoid> struct hld_disjoint_sparse_table {
    using MX = Monoid;
    using X = typename MX::ValueT;

    const hld &h;

    disjoint_sparse_table<MX> st;
    disjoint_sparse_table<monoid_reverse_monoid<MX>> rst;

    explicit hld_disjoint_sparse_table(const hld &g) : h(g) { build(); }

    template <typename F> hld_disjoint_sparse_table(const hld &g, F f) : h(g) { build(f); }

    explicit hld_disjoint_sparse_table(const hld &g, const std::vector<X> &v) : h(g) { build(v); }
...
#endif // LIB_HLD_DISJOINT_SPARSE_TABLE_HPP
