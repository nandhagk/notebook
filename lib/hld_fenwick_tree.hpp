...
struct hld_fenwick_tree {
        ...
        explicit hld_fenwick_tree(const hld& g): h(g) {
                build();
        }

        template <typename F>
        hld_fenwick_tree(const hld& g, F f): h(g) {
                build(f);
        }

        explicit hld_fenwick_tree(const hld& g, const std::vector<X> &v): h(g) {
                build(v);
        }
        ...
        X prod_subtree(i32 u) {
                return ft.prod(h.tin[u], h.tin[u] + h.sz[u]);
        }
};
