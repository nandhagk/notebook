...
struct wavelet_matrix_segment_tree {
	...
	std::vector<segment_tree<Monoid>> sg;
        ...
        explicit wavelet_matrix_segment_tree(const std::vector<T> &v) {
                build(v);
        }

        wavelet_matrix_segment_tree(const std::vector<T> &v, std::vector<X> s) {
                build(v, s);
        }

        void build(const std::vector<T> &v) {
                build(v, std::vector<X>(v.size(), MX::unit()));
        }
