#ifndef LIB_MAXIMUM_WEIGHTED_MATCHING_HPP
#define LIB_MAXIMUM_WEIGHTED_MATCHING_HPP 1
...
private:
    ...
    private:
        struct Node {
            Node()
                : prev(-1) {} // "prev < 0" means the node is unused.
            Node(Key v)
                : key(v), child(0), next(0), prev(0) {}
            Key key;
            int child, next, prev;
        };

    public:
        PairingHeaps(int H, int N)
            : heap(H), node(N) {
    ...
    private:
        ...
        int merge(int l, int r) {
            if (!l) return r;
            if (!r) return l;
            if (node[l].key > node[r].key) std::swap(l, r);
...
public:
    MaximumWeightedMatching(int N, const std::vector<InputEdge> &in)
        : N(N),
          B((N - 1) / 2),
          S(N + B + 1),
          ofs(N + 2),
    ...
    std::vector<InputEdge> get_matching_edges() {
        std::vector<InputEdge> ans;
        for (int u = 1; u <= N; ++u) {
            if (mate[u] > u) {
                cost_t max_c = 0;
...
private:
    ...
    void swap_blossom(int a, int b) {
        ...
        std::swap(heavy[a], heavy[b]);
        if (heavy[a] == a) heavy[a] = b;
        std::swap(link[a], link[b]);
        std::swap(mate[a], mate[b]);
        std::swap(potential[a], potential[b]);
        std::swap(lazy[a], lazy[b]);
        std::swap(time_created[a], time_created[b]);
    ...
    bool adjust_dual_variables(int root) {
        ...
        // -- events --
        cost_t time_next = std::min(std::min(time1, time2), std::min(time3, time4));
...
private:
    ...
    Queue<int> que;
    std::vector<int> mate, surface, base;
    std::vector<Link> link;
    std::vector<Label> label;
    std::vector<cost_t> potential;

    std::vector<int> unused_bid;
    int unused_bid_idx_;
    std::vector<Node> node;

    // for O(nm log n) implementation
    std::vector<int> heavy, group;
    std::vector<cost_t> time_created, lazy, slack;
    std::vector<int> best_from;
...
#endif // LIB_MAXIMUM_WEIGHTED_MATCHING_HPP
