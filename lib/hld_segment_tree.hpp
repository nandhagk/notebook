...
struct hld_segment_tree {
	...
	segment_tree<MX> st;
	segment_tree<monoid_reverse_monoid<MX>> str;

        explicit hld_segment_tree(const Tree &g):
		h(g) {
                build();
        }

        template <typename F>
        hld_segment_tree(const Tree& g, F f):
		h(g) {
                build(f);
        }

        explicit hld_segment_tree(const Tree& g, const std::vector<X> &v):
		h(g) {
                build(v);
        }
