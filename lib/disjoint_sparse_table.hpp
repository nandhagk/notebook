...
template <typename Monoid, is_monoid_t<Monoid> * = nullptr> struct disjoint_sparse_table {
    using MX = Monoid;
    ...
    explicit disjoint_sparse_table(i32 m) { build(m); }

    explicit disjoint_sparse_table(const std::vector<X> &v) { build(v); }

    template <typename F> disjoint_sparse_table(i32 m, F f) { build(m, f); }
