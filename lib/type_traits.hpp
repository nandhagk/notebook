...
template <typename T>
using is_unsigned_integral = std::conditional_t<
	std::is_integral_v<T> && std::is_unsigned_v<T>, std::true_type, std::false_type>;
...
template <typename T>
using make_signed = std::conditional_t<std::is_same_v<T, u128>, i128, std::conditional_t<std::is_same_v<T, u64>, i64, i32>>;
...
template <typename T>
struct has_op<T, std::enable_if_t<
	 std::is_same_v<
		decltype(T::op(std::declval<typename T::ValueT>(), std::declval<typename T::ValueT>())), 
		typename T::ValueT>>>
	: std::true_type {};
...
template <typename T>
struct has_unit<T, std::enable_if_t<std::is_same_v<decltype(T::unit()), typename T::ValueT>>>
	: std::true_type {};
...
template <typename T>
struct has_inv<T, std::enable_if_t<
	 std::is_same_v<
		decltype(T::inv(std::declval<typename T::ValueT>())), 
		typename T::ValueT>>>
	: std::true_type {};
