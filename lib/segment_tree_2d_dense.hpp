#ifndef LIB_SEGMENT_TREE_2D_DENSE_HPP
#define LIB_SEGMENT_TREE_2D_DENSE_HPP 1
...
struct segment_tree_2d_dense {
	...
	segment_tree_2d_dense(i32 h, i32 w) {
		build(h, w);
	}

	explicit segment_tree_2d_dense(const std::vector<std::vector<X>> &v) {
		build(v);
	}

	template <typename F>
	segment_tree_2d_dense(i32 h, i32 w, F f) {
		build(h, w, f);
	}
...
#endif // LIB_SEGMENT_TREE_2D_DENSE_HPP
