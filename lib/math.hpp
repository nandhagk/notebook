...
template <u32 m> constexpr u32 mul(u32 a, u32 b) { return static_cast<u32>((u64(a) * b) % m); }

template <u64 m> constexpr u64 mul(u64 a, u64 b) { return static_cast<u64>((u128(a) * b) % m); }

template <typename T> constexpr T safe_mod(T x, T m) {
    x %= m;
    if (x < 0) x += m;
    return x;
}

template <typename T, is_signed_integral_t<T> * = nullptr> constexpr std::pair<T, T> inv_gcd(T a, T b) {
    a = safe_mod(a, b);
    if (a == 0) return {b, 0};
...
template <typename T, is_signed_integral_t<T> * = nullptr> constexpr T inv(T a, T b) {
...
template <typename T> constexpr T modpow(T a, i64 n, T p) {
